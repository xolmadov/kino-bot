...
async def admin_series_code(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🔢 <b>Nechta qism yuklaysiz hozir?</b> (1-100)\n💡 Keyin yana qo'shish mumkin!"
    )
...
async def admin_save_channel(message: Message, state: FSMContext):
    ...
    try:
        ...
        channel_link, channel_name = parts[1], parts[2]
        if not channel_link.startswith("https://t.me/"):
            raise ValueError()
        channels = await load_channels()
        if len(channels) >= 10:
            await message.answer("❌ Maksimal 10 ta kanal!", reply_markup=get_admin_keyboard(uid))
            await state.set_state(AdminFSM.idle)
            return
        ...
        await message.answer(f"❌ Xato: <code>{e}</code>", reply_markup=get_admin_keyboard(uid))
    await state.set_state(AdminFSM.idle)
...
async def main():
    ...
    admins = await load_admins()
    print(f"🔧 Adminlar: {len(admins)} ta")
    channels = await load_channels()
    movies = await load_data()
    series = await load_series()
