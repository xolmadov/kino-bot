...
CHANNELS = []
EP_PER_PAGE = 10  # Har sahifada nechta qism
...
async def search_content(query: str) -> list:
    """
    Kino va seriallardan nom yoki kod bo'yicha qidiradi.
    Natija: [{"code": ..., "title": ..., "kind": "movie"/"series", "extra": ...}, ...]
    """
    ...
    for code, v in movies.items():
        ...
        if query in code or query in title.lower():
            results.append({
                "code": code,
...
# ============================================================
# QISMLAR SAHIFASINI ALMASHTIRISH (callback)
# ============================================================
...
# ============================================================
# NOOP CALLBACK (sahifa raqami tugmasi)
# ============================================================
...
# ============================================================
# QIDIRUV NATIJASINI TANLASH (callback)
# ============================================================
@dp.callback_query(F.data.startswith("pick_"))
async def pick_content(callback: types.CallbackQuery):
    # pick_movie_<code> yoki pick_series_<code>
    ...
    if kind == "movie":
        ...
        if not episodes:
            await callback.message.answer("❌ Serial qismlari topilmadi.")
            return
        ...
        try:
            ...
            await callback.message.answer("❌ Serial yuborishda xato.")
            return
        ...
        if total > 1:
            ...
            await callback.message.answer(
                f"📺 <b>{title}</b> — barcha qismlar ({total} ta)\n"
                f"📄 Sahifa 1/{total_pages} (1-{min(EP_PER_PAGE, total)} qismlar):",
                reply_markup=kb
            )
...
async def admin_receive_episode(message: Message, state: FSMContext):
    ...
    if current >= total_to_upload:
        ...
        await message.answer(
            ...
            "💡 <b>➕ Serialga qism qo'shish</b> orqali yangi qismlar qo'shishingiz mumkin!",
            reply_markup=admin_keyboard
        )
        await state.set_state(AdminFSM.idle)
    else:
...
async def admin_add_ep_count(message: Message, state: FSMContext):
    ...
    await message.answer(f"📤 <b>{start_from}-qismni yuboring:</b>")
...
async def admin_add_ep_file(message: Message, state: FSMContext):
    ...
    if uploaded >= total:
        ...
        await message.answer(
            ...
            f"📤 <b>{next_num}-qismni yuboring</b> ({uploaded}/{total}):"
        )
...
async def admin_add_ep_wrong(message: Message, state: FSMContext):
    data = await state.get_data()
    start_from = data.get("add_ep_start_from", 1)
    uploaded = data.get("add_ep_uploaded", 0)
    await message.answer(f"⚠️ {start_from + uploaded}-qism uchun video yoki fayl yuboring!")
...
# ============================================================
# FOYDALANUVCHI — KOD YOKI NOM KIRITISH
# ============================================================
...
async def user_enter_code(message: Message, state: FSMContext):
    ...
    if len(results) == 1:
        # Bitta natija — to'g'ridan yuborish
        r = results[0]
        if r["kind"] == "movie":
            movie = movies[r["code"]]
            file_id = movie["file_id"] if isinstance(movie, dict) else movie
    ...
    # Bir nechta natija — tanlash tugmalari
    buttons = []
...
async def send_series_first(message: Message, code: str, s: dict):
    """Serial 1-qismini yuboradi va sahifalangan tugmalar chiqaradi"""
    title = s.get("title", "Serial")
    episodes = s.get("episodes", [])
    total = len(episodes)
